...
def rotmat_teamrr_pass(d, bs, G) : 
    thetas = torch.nn.Parameter(torch.zeros(getThetaCount(d,bs)).normal_(0, 2*np.pi))
    X = torch.zeros((d, bs)).normal_(0, 1)
    torch.cuda.synchronize()

    # Start timing of forward and backwards pass. 
    t0 = time.time()
    Y = RotMatOptFunction.apply(X, thetas)
    torch.autograd.backward(Y, G)
    torch.cuda.synchronize()

    return time.time() - t0

def rotmat_pass(d, bs, G) : 
    thetas = torch.nn.Parameter(torch.zeros(getThetaCount(d,bs)).normal_(0, 2*np.pi))
    X = torch.zeros((d, bs)).normal_(0, 1)
    torch.cuda.synchronize()
    ...
    # Start timing of forward and backwards pass. 
    t0 = time.time()
    Y = RotMatFunction.apply(X, thetas)
    torch.autograd.backward(Y, G)
    torch.cuda.synchronize()

    return time.time() - t0

def run_rotmat(d, bs, repeats, teamRR=True, m=None): 

    times = []
    rotmat = rotmat_teamrr_pass if teamRR else rotmat_pass
    if m is None: 
        if d <= 128: 	m = 16 
        else:		 	m = 32
...
def run_regular_linear(d, bs, repeats, teamRR=True, m=None): 
